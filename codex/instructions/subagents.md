...
1. 默认最多并发 3 个子 Agent，并受运行时更低上限约束。只并行互不依赖且能明显缩短墙钟时间的任务。
2. 子 Agent 不自行递归分发；新增子任务由主 Agent 统一管理。
3. 派发前写明扫描范围、收敛条件和等待预算；达到验收标准立即返回。
4. 主 Agent 等待期间推进独立工作，不重复扫描同一范围。
5. 等待预算到期时检查实际进展：有新证据才记录续期理由；没有进展则回收已有证据并缩小范围。
6. 同一子任务失败后，只有输入、方法或外部条件发生有效变化时才重试，默认最多一次。
7. 中断、无返回或缺少关键证据的子任务不能视为完成。主 Agent 核验关键证据，但不完整重做已通过验收的子任务。
